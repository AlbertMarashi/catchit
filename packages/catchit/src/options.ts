...
const forceColor = new chalk.Instance({level: Math.max(chalk.level, 1) as 0 | 1 | 2 | 3 });
...
const plainTheme = cloneDeepWith(colorTheme, (value: any) => {
...
export default {
	...
	get maxDepth() {
		return Math.max(3, inspect.defaultOptions.depth as number);
	},
	theme
};
